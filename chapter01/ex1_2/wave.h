...
void mono_wave_write(MONO_PCM *pcm, char *file_name)
{
  ...
  for (n = 0; n < pcm->length; n++)
  {
    s = (pcm->s[n] + 1.0) / 2.0 * 65536.0;  /* 左辺のsはmono_wave_write関数内で定義したdouble型の変数, 右辺のsはMONO_PCM構造体で定義した, サンプルごとの音データの配列(double*型) */
...
void stereo_wave_read(STEREO_PCM *pcm, char *file_name)
{
  ...
  for (n = 0; n < pcm->length; n++)
  {
    fread(&data, 2, 1, fp);              /* 音データ（Lチャンネル）�̓ǂݎ�� */
    pcm->sL[n] = (double)data / 32768.0; /* 音データを-1以上1未満の範囲に正規化する */

    fread(&data, 2, 1, fp);              /* 音データ（Rチャンネル）�̓ǂݎ�� */
